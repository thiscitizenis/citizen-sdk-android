...
public class RespondTokenTask extends AsyncTask<Object, Void, Integer> {
    private static final String TAG = RespondTokenTask.class.getSimpleName();

    public interface AsyncResponse {
        void respondTokenTaskFinished(Integer statusCode, Token token);
    }
    ...
    public RespondTokenTask(AsyncResponse delegate, TokenService tokenService) {
    ...
    protected Integer doInBackground(Object... params) {
        ...
        try {
            if (token.getTokenStatus().equals((TokenStatus.GRANTED))) {
                this.token = tokenService.grantToken(token, apiKey, mnemonic);
            } else if (token.getTokenStatus().equals(TokenStatus.DECLINED)) {
                this.token = tokenService.declineToken(token, apiKey, mnemonic);
            } else {
                statusCode = Constant.CITIZEN_REST_INVALID_TOKEN_STATUS;
            }

            if (this.token != null && statusCode != Constant.CITIZEN_REST_INVALID_TOKEN_STATUS) {
    ...
    protected void onPostExecute(Integer statusCode) {
        delegate.respondTokenTaskFinished(statusCode, token);
    }
}
